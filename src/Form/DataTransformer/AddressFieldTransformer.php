...
class AddressFieldTransformer implements DataTransformerInterface
{
    ...
    public function __construct(AddresseRepository $addressRepo)
    {
    ...
    public function transform($value)
    {
        ...
        if (!$value instanceof Addresse) {
            throw new \LogicException('AddressFieldTransformer can only be used for Addresses!');
        }
    ...
    public function reverseTransform($value)
    {
        ...
        $address = $this->addressRepo->findOneBy(['id' => (int) substr($rest2, 1) ]);
        if (!$address) {
            throw new TransformationFailedException(sprintf('No address found with id "%s"', substr($rest2, 1)));
        }
